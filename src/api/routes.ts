 ...
 */
router.post('/weather', validateRequest(WeatherQuerySchema), async (req: Request, res: Response) => {
  try {
    const { query, units, summary } = req.body;
    
    // Create a temporary CLI instance to reuse existing logic
    const weatherCLI = new WeatherCLI({ units: units || 'imperial', debug: false });
    ...
    if (intent.useIpLocation) {
      try {
        ...
        throw new ServiceError('Failed to resolve IP location', 'geoip', error);
      }
    }
    ...
    if (summary) {
      try {
        aiSummary = await openAIService.generateSummary(currentWeather, intent.extras || []);
      } catch (error) {
 ...
 */
router.get('/forecast', validateRequest(LocationQuerySchema), async (req: Request, res: Response) => {
  try {
    ...
    const [currentData, forecastData] = await Promise.all([
      ...
      openWeatherService.getForecast(city, units || 'imperial')
    ]);
    
    // Limit forecast to requested days
    const limitedForecast = forecastData.forecast.slice(0, days || 3);
    
    // Generate summary if requested
    let aiSummary: string | undefined;
    if (summary) {
      try {
        aiSummary = await openAIService.generateSummary(currentData, ['forecast']);
      } catch (error) {
 ...
 */
router.post('/compare', validateRequest(CompareQuerySchema), async (req: Request, res: Response) => {
  try {
    ...
    if (validWeatherData.length === 0) {
      throw new ServiceError('Failed to fetch weather data for any city', 'openweather');
    }
    ...
    if (summary && validWeatherData.length > 0) {
      try {
        aiSummary = await openAIService.generateSummary(validWeatherData[0], ['compare']);
      } catch (error) {
